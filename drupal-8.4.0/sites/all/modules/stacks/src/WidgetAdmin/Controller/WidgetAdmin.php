...
/**
 * @file
 * Contains \Drupal\stacks\Controller\admin\StacksAdminAJAX.
 */
...
class WidgetAdmin extends ControllerBase {
  ...
  static public function validateWidgetName($current = "", $search = "") {
    ...
    if($search != '') {
      ...
      $matches = $autocomplete->getMatches('widget_instance_entity', 'default', array('match_operator' => '='), $search);
  ...
  static public function nodeSubmitStatus(AjaxResponse $response, $status) {
    if($status) {
      $response->addCommand(new InvokeCommand('#edit-actions input', 'removeAttr', array('disabled')));
      $response->addCommand(new InvokeCommand('#edit-actions input', 'removeClass', array('is-disabled')));
    }
    else {
      $response->addCommand(new InvokeCommand('#edit-actions input', 'attr', array('disabled', 'true')));
      $response->addCommand(new InvokeCommand('#edit-actions input', 'addClass', array('is-disabled')));
    }
  }
  ...
  static public function ajaxForm() {
    ...
    $response->addCommand(new InvokeCommand('#widget-form-' . $delta, 'attr', array('data-haschanged', 'false')));
  ...
  static public function ajaxFormDelete() {
    ...
    $response->addCommand(new InvokeCommand('#widget-instance-' . $delta . ' input', 'val', array('')));
    ...
    $element['undo'] = [
      ...
      '#url' => \Drupal\Core\Url::fromRoute('stacks.admin.ajax_undo', array(), ['query' => $undo_link_options]),
      '#attributes' => [
...
//    ];
  ...
  static public function dGridPreview(&$form, $form_state) {
    ...
    if ($tsort == 'title_asc') {
      unset($view->display_handler->handlers['sort']['created']);
    }
    else {
      unset($view->display_handler->handlers['sort']['title']);
    }
